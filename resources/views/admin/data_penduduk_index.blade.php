...
<style>
    ...
    .header-bar { display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; margin-bottom: 2rem; border-bottom: 1px solid var(--color-border); padding-bottom: 1rem; gap: 1rem; }
    ...
    .btn-tambah-data { background-color: var(--color-primary); color: var(--color-text-light); padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 600; white-space: nowrap; }
    .stat-cards-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem; margin-bottom: 2.5rem; }
    ...
    .gender-tag.laki-laki { background-color: #87CEEB; }
    .gender-tag.perempuan { background-color: #F08080; }
        ...
        <div class="card-content">
            ...
            <div class="card-body">
                ...
                <p><strong>Alamat:</strong>
                    ...
                    {{ $p->kelurahan ?? 'Sukorame' }}
                </p>
                <p><strong>TTL:</strong> {{ $p->tempat_lahir ?? '-' }}, {{ $p->tanggal_lahir ? $p->tanggal_lahir->format('d-m-Y') : '-' }}</p>
                <p><strong>Pekerjaan:</strong> {{ $p->pekerjaan ?? '-' }}</p>
            </div>
        </div>
        <div class="card-actions">
            <a href="{{ route('admin.penduduk.edit', $p->nik) }}" class="btn-aksi edit">Edit</a>
            <form action="{{ route('admin.penduduk.destroy', $p->nik) }}" method="POST" style="display:inline;">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn-aksi hapus" onclick="return confirm('Yakin ingin menghapus data {{ $p->nama_lengkap }}?')">Hapus</button>
            </form>
        </div>
    </div>
    @empty
