...
<style>
    ...
    .photo-info { font-size: 0.85rem; color: #aaa; }
    
    /* --- STATUS PENCARIAN --- */
    .loading-status { font-size: 0.85rem; margin-top: 8px; font-weight: 600; }
    .st-searching { color: var(--color-warning); display: none; } 
    .st-found { color: var(--color-primary); display: none; } 
    .st-error { color: var(--color-danger); display: none; } 
</style>
@endpush
...
<div class="form-container">
    ...
    <div class="form-card">
        ...
        <form action="{{ route('admin.bangunan.update', $bangunan->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select name="kategori" id="kategori" class="form-select" required>
                    ...
                    @foreach(['Makanan & Minuman', 'Perbelanjaan', 'Akomodasi', 'Perumahan', 'Layanan Publik & Administrasi', 'Pemerintahan & Sipil', 'Kesehatan', 'Pendidikan', 'Rekreasi, Seni & Budaya', 'Alam & Lingkungan', 'Otomotif & Transportasi', 'Agama & Spiritual'] as $kat)
                        <option value="{{ $kat }}" {{ old('kategori', $bangunan->kategori) == $kat ? 'selected' : '' }}>
                            {{ $kat }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea name="alamat" id="alamat" class="form-control" rows="3" required>{{ old('alamat', $bangunan->alamat) }}</textarea>
                ...
                <div class="loading-status">
                    ...
                    <span id="msg-error" class="st-error">❌ Lokasi tidak ditemukan. Coba kurangi detail alamat.</span>
                </div>
            </div>
            ...
            <div class="grid-2">
                ...
                <div class="form-group">
                    ...
                    <select name="rt_id" id="rt_id" class="form-select" required>
                        {{-- Opsi RT akan dimuat otomatis oleh JS berdasarkan RW yang terpilih --}}
            ...
            <div class="form-group">
                ...
                <textarea name="deskripsi" id="deskripsi" class="form-control" rows="2">{{ old('deskripsi', $bangunan->deskripsi) }}</textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="current-photo-container">
                    <img src="{{ asset('storage/' . $bangunan->foto) }}" alt="Foto Lama">
            ...
            <div class="grid-2" style="background: rgba(0,0,0,0.2); padding: 10px; border-radius: 8px;">
                <div class="form-group" style="margin-bottom:0;">
                    ...
                    <input type="text" name="latitude" id="latitude" class="form-control"
                        value="{{ old('latitude', $bangunan->latitude) }}" readonly style="background:#0f2238; color:#aaa;" required>
                </div>
                <div class="form-group" style="margin-bottom:0;">
                    <label style="font-size:0.85rem;">Longitude (Otomatis)</label>
                    <input type="text" name="longitude" id="longitude" class="form-control"
                        value="{{ old('longitude', $bangunan->longitude) }}" readonly style="background:#0f2238; color:#aaa;" required>
                </div>
            </div>
...
[Image of Map marker icon]
 ...
 Pin saat ini menunjukkan lokasi bangunan.<br>
            Anda bisa menggeser pin secara manual untuk koreksi posisi.
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // --- 1. SETUP MAP (Dengan Koordinat Existing) ---
        // Ambil nilai dari input (yang sudah diisi old atau data DB)
        let curLat = parseFloat(document.getElementById('latitude').value);
        let curLng = parseFloat(document.getElementById('longitude').value);

        // Fallback jika data kosong (seharusnya tidak terjadi di edit)
        if (isNaN(curLat)) curLat = -7.812345;
        if (isNaN(curLng)) curLng = 112.012345;

        const map = L.map('map').setView([curLat, curLng], 17); // Zoom lebih dekat untuk edit
        ...
        // --- 3. LOGIKA RW/RT (Pre-fill Data) ---
        const rwSelect = document.getElementById('rw_id');
        const rtSelect = document.getElementById('rt_id');
        
        // Ambil RT ID yang tersimpan (dari old input atau database)
        const savedRtId = "{{ old('rt_id', $bangunan->rt_id) }}";

        // Jalankan fetch saat halaman dimuat jika RW sudah terpilih
        if (rwSelect.value) {
            fetchRT(rwSelect.value, savedRtId);
        }

        rwSelect.addEventListener('change', function() { 
            fetchRT(this.value, null); // Jika RW berubah, reset RT selection
        });

        function fetchRT(rwId, selectedRtId) {
            ...
            if (rwId) {
                fetch(`/api/get-rt-by-rw/${rwId}`)
                    ...
                    .then(data => {
                        ...
                        data.forEach(rt => {
                            // Cek jika ID sama dengan data tersimpan
                            let isSelected = (rt.id == selectedRtId) ? 'selected' : '';
                            rtSelect.innerHTML += `<option value="${rt.id}" ${isSelected}>RT ${rt.nomor_rt}</option>`;
                        });
                        rtSelect.disabled = false;
                    })
                    .catch(() => rtSelect.innerHTML = '<option value="">Gagal memuat RT</option>');
            } else {
